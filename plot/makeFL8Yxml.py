...
class srcList:
    ...
    def Print(self):
        ...
        print('Selecting %s degrees around (ra,dec)=(%s,%s)' % (self.roi[2],
                                                                self.ra,
                                                                self.dec))
    ...
    #maxRad (float) -- optional, absolute maximum radius beyond which sources are fixed, this may be necessary when doing binned analysis and a Gariable source beyond radLim would be set free but this source is beyond the boundaries of the square region used for the binned likelihood
    ...
    def makeModel(
            self,
            GDfile="$(FERMI_DIR)/refdata/fermi/galdiffuse/gll_iem_v06.fits",
            GDname='gll_iem_v06',
            ISOfile="$(FERMI_DIR)/refdata/fermi/galdiffuse/iso_P8R2_SOURCE_V6_v06.txt",
        ...
        if self.maxRad < self.radLim:
            print("NOTE: maxRad (%.1f deg) is less than radLim (%.1f deg), meaning maxRad parameter is useless" % (
                self.maxRad, self.radLim))
        self.var = varFree
        self.psF = psForce
        self.E2C = E2CAT
        self.nO = normsOnly
        extDir = (extDir if extDir != '' else '$(LATEXTDIR)/Templates/'
                  )  #make sure the default for FSSC STs is correct
        ...
        if makeRegion:
            ...
            rhold = rhold[0].split(
                '/'
            )[-1]  #just in case the user has specified the full path for the output XML model file
            wd = (os.getcwd() if wd == '' else wd)
            self.regFile = wd + '/ROI_'
            for r in rhold:
                self.regFile += r
            self.regFile += '.reg'
        print('Creating file and adding sources from FL8Y')
...
print("This is makeFL8Yxml version 01r01.")
print("The default diffuse model files and names are for pass 8 and assume you have v11r5p3 of the Fermi Science Tools or higher.")
...
def addSrcsXML(sL, GD, GDn, ISO, ISOn, oldNames=False):
    ...
    outputXml = minidom.getDOMImplementation().createDocument(
        None, 'source_library', None)
    ...
    for src in catalog:
        if src.getAttribute('type') == 'PointSource':
            for p in src.getElementsByTagName('spatialModel')[
            ...
            try:
                ...
                for p in src.getElementsByTagName('spatialModel')[
                    ...
                    if p.getAttribute('name') == 'DEC':
                        srcDEC = float(p.getAttribute('value'))
        dist = angsep(
            sL.roi[0], sL.roi[1], srcRA, srcDEC
        )  #check that source is within ROI radius + 10 degress of ROI center
        ...
        if dist <= sL.roi[2] + sL.ER:
            ...
            Ext = (True if (src.getAttribute('type') == 'DiffuseSource'
                            and not sL.psF) else False)
            if Ext and str(src.getAttribute('name')) == 'EXTRosette':
                Ext = False
            #comment out the next two lines later, for now FSSC STs can't handle these two spatial models
            #and doesn't have Rosette nebula spatial template
            ##if Ext and str(src.getElementsByTagName('spatialModel')[0].getAttribute('type')) in ['RadialDisk','RadialGaussian']:
            ##Ext=False
            sname = src.getAttribute('name')
            #comment out the stuff below for now...likely add something backf or official 4FGL release
            ...
            #if dist>=sL.roi[2] or dist>=sL.maxRad or fixAll:#uncomment later
            if dist >= sL.roi[2] or dist >= sL.maxRad:
                Sources[sname]['free'] = False
                specOut.setAttribute(
                    'apply_edisp',
                    'false')  #source is fixed, so never apply edisp
                ##if str(specType)!='PLSuperExpCutoff2':
                for p in specPars:
                    specOut.appendChild(
                        parameter_element("0",
                ...
                ##b=None
                ##for p in specPars:
                ##if str(p.getAttribute('name'))=='Expfactor':
                ##a=float(p.getAttribute('value'))*float(p.getAttribute('scale'))
                ##if str(p.getAttribute('name'))=='Index2':
                ##b=float(p.getAttribute('value'))*float(p.getAttribute('scale'))
                ##cut=(1./a)**(1./b)
                ##for p in specPars:
                ##if str(p.getAttribute('name'))=='Expfactor':
                ##specOut.appendChild(parameter_element("0","Cutoff","100000","100","1","%s"%cut))
                ##else:
                ...
                if sL.var and varIdx >= 72.44:  #update value later
                    Sources[sname]['free'] = True
                    specOut.setAttribute('apply_edisp', ed)
                    ##if str(specType)!='PLSuperExpCutoff2':
                    for p in specPars:
                        freeFlag = ("1" if p.getAttribute('name') == spec[0]
                                    .getAttribute('normPar') else "0")
                        specOut.appendChild(
                            parameter_element(
                                "%s" % freeFlag,
                                "%s" % str(p.getAttribute('name')),
                ...
                ##if str(specType)!='PLSuperExpCutoff2':
                for p in specPars:
                    freeFlag = ("1" if p.getAttribute(
                        'name') == spec[0].getAttribute('normPar') or
                                (not sL.nO and p.getAttribute('free') == "1")
                                else "0")
                    #if str(p.getAttribute('name')) in normNames or (not sL.nO and str(p.getAttribute('name')) in freePars):
                    specOut.appendChild(
                        parameter_element("%s" % freeFlag,
                                          "%s" % str(p.getAttribute('name')),
                                          "%s" % str(p.getAttribute('max')),
                                          "%s" % str(p.getAttribute('min')),
                                          "%s" % str(p.getAttribute('scale')),
                ...
                ##for p in specPars:
                ##if str(p.getAttribute('name'))=='Expfactor':
                ##freeFlag=("1" if (not sL.nO and p.getAttribute('free')=="1") else "0")
                ##specOut.appendChild(parameter_element("%s"%freeFlag,"Cutoff","100000","100","1","%s"%cut))
                ##else:
            ...
            if Ext:
                ...
                if str(spatType) == 'SpatialMap':
                    ...
                    print('Extended source %s in ROI, make sure %s is the correct path to the extended template.' % (
                        sname, efile))
                    ...
                    for p in spatPars:  #for radial disks and gaussians, can just do the following
                        spatialOut.appendChild(
                            parameter_element(
                                ...
                                "%s" % str(p.getAttribute('scale')),
                                "%s" % str(p.getAttribute('value'))))
                        print('Extended source %s in ROI, with %s spatial model.' % (
                            sname, str(spatType)))
                ...
                spatialOut.appendChild(
                    parameter_element("0", "RA", "360.0", "-360.0", "1.0",
                                      "%.4f" % srcRA))
                spatialOut.appendChild(
                    parameter_element("0", "DEC", "360.0", "-360.0", "1.0",
                                      "%.4f" % srcDEC))
    ...
    galspec.appendChild(
        parameter_element("1", "Prefactor", "10", "0", "1", "1"))
    if sL.GIF:
        galspec.appendChild(
            parameter_element("1", "Index", "1", "-1", "1", "0"))
    else:
        galspec.appendChild(
            parameter_element("0", "Index", "1", "-1", "1", "0"))
    galspec.appendChild(
        parameter_element("0", "Scale", "1e6", "2e1", "1", "100"))
    ...
    galspatial.appendChild(
        parameter_element("0", "Normalization", "1e3", "1e-3", "1", "1"))
    ...
    isospec.appendChild(
        parameter_element("1", "Normalization", "10", "0.01", "1", "1"))
    isospatial = outputXml.createElement('spatialModel')
    isospatial.setAttribute('type', 'ConstantValue')
    isospatial.appendChild(
        parameter_element("0", "Value", "10", "0", "1", "1"))
    ...
    outStr = filter(lambda xmlStr: len(xmlStr) and not xmlStr.isspace(),
                    xmlStr)
...
def addSrcsFITS(sL, GD, GDn, ISO, ISOn, oldNames):
    ...
    step = (
        sL.roi[2] + sL.ER
    ) / 5.  #divide ROI radius plus ExtraRadius degrees into 5 steps for ordering of sources
    i = 1
    radii = []
    ptSrcNum = 0
    extSrcNum = 0
    Sources = {
    }  #dictionary for sources, useful for creating region file later.
    while i < 6:
    ...
    for x in radii:
        ...
        for n, f, r, d, p, pli, lpi, lpb, pleci, plecef, plecei, t, TS, En, in zip(
                name, flux, ra, dec, pivot, plIndex, lpIndex, lpbeta,
                plecIndex, plecexpFact, plecexpIndex, spectype, Sigvals,
                EName):
            vi = -1  #remove later
            ...
            if r == sL.roi[0] and d == sL.roi[1]:
                dist = 0.0
            if (dist < x and dist >= x - step) or (x == sL.roi[2] + 10.
                                                   and dist == x):
                ###comment out the 6 lines below later
                ##if E:
                ##efunc=None
                ##for EXTNAME,EXTFUNC in zip(extName,extFunc):
                ##if En==EXTNAME:
                ##efunc=EXTFUNC
                ###print(En,efunc)
                ##if E and not sL.psF and not En=='Rosette' and not (efunc in ['RadialDisk','RadialGauss']):
                if E and not sL.psF and En != 'Rosette':  #uncomment this later when FSSC STs can deal with rosette nebula and two new spatial models
                ...
                if t == 'PowerLaw':
                    ...
                    spec, free = PLspec(sL, f, pli, p, dist, TS, vi, False)
                    ...
                    spec, free = LPspec(sL, f, lpi, p, lpb, dist, TS, vi)
                else:
                    ##spec,free=COspec(sL,f,pleci,p,plecef,plecei,dist,TS,vi)
                    spec, free = CO2spec(sL, f, pleci, p, plecef, plecei, dist,
                                         TS, vi)
                if E and not sL.E2C:
                    Sources[En]['free'] = free
                else:
                    Sources[n]['free'] = free
                if E and not sL.psF and En != 'Rosette':
                    efile = None
                    efunc = None
                    eSize = None
                    eR = None
                    eD = None
                    for EXTNAME, EXTFILE, EXTFUNC, EXTSIZE, EXTRA, EXTDEC in zip(
                            extName, extFile, extFunc, extSize, extRa, extDec):
                        if En == EXTNAME:
                            efunc = EXTFUNC
                            efunc = ('RadialGaussian'
                                     if efunc == 'RadialGauss' else efunc)
                            if efunc == 'SpatialMap':
                    ...
                    if efunc == 'SpatialMap':
                        ...
                        ###comment out the next four lines and uncomment the stuff below when the new
                        ###spatial models can be handled by FSSC STs
...
def BuildRegion(sL, Sources):
    myreg = open(
        sL.regFile, 'w'
    )  #note that this will overwrite previous region files of the same name
    myreg.write(
        '# Region File format: DS9 version ?'
    )  #I don't actually know, but I think it's one of the later ones, need to verify
    ...
    for k in Sources.keys():
        ...
        if src['E']:  #if the source is extended, always have the point be a "big" box
            myreg.write(
                '\nJ2000;point(%.3f,%.3f) # point = box 18 color = %s text={%s}'
                % (src['ra'], src['dec'], color, k))
        else:  #if the source is a point source, choose the point type based on spectral model
            ptype = ('cross'
                     if src['stype'] == 'PLSuperExpCutoff' else 'diamond'
                     if src['stype'] == 'LogParabola' else 'circle')
            myreg.write(
                '\nJ2000;point(%.3f,%.3f) # point = %s 15 color = %s text={%s}'
                % (src['ra'], src['dec'], ptype, color, k))
    myreg.close()
    return
...
def PLspec(sL, f, i, p, dist, TS, vi, fixAll):
    ...
    if (dist > sL.roi[2] or fixAll) or (dist > sL.maxRad) or (
            dist > sL.radLim and
        (vi < 72.44 or not sL.var)) or (TS < sL.sig and
                                        (vi < 72.44 or not sL.var)):
        spec = '\t<spectrum type="PowerLaw" apply_edisp="false">\n'
    else:
        spec = '\t<spectrum type="PowerLaw" apply_edist="%s">\n' % sL.ed
...
def PL2spec(sL, F, i, dist, TS, vi):
    fscale = int(floor(log10(F)))
    if dist > sL.roi[2] or (dist > sL.maxRad) or (
            dist > sL.radLim and
        (vi < 72.44 or not sL.var)) or (TS < sL.sig and
                                        (vi < 72.44 or not sL.var)):
        spec = '\t<spectrum type="PowerLaw2" apply_edisp="false">\n'
    else:
        spec = '\t<spectrum type="PowerLaw2" apply_edist="%s">\n' % sL.ed
    spec += '\t<!-- Source is %s degrees away from ROI center -->\n' % dist
...
def COspec(sL, f, i, p, a, ei, dist, TS, vi):
    ...
    if dist > sL.roi[2] or (dist > sL.maxRad) or (
        ...
        spec = '\t<spectrum type="PLSuperExpCutoff" apply_edist="%s">\n' % sL.ed
    spec += '\t<!-- Source is %s degrees away from ROI center -->\n' % dist
    i = (
        i if i >= 0 else 2.
    )  #some pulsars with index1 < 0 assuming standard convention, means rising counts spectrum at low E, odd
    if (dist > sL.roi[2]):  #if beyond ROI, shouldn't attempt to fit parameters
        spec += '\t<!-- Source is outside ROI, all parameters should remain fixed -->\n'
        free = False
        spec += '\t\t<parameter free="0" max="1e4" min="1e-4" name="Prefactor" scale="1e%i" value="%s"/>\n' % (
...
def CO2spec(sL, f, i, p, a, ei, dist, TS, vi):
    ...
    if dist > sL.roi[2] or (dist > sL.maxRad) or (
        ...
        (vi < 72.44 or not sL.var)) or (TS < sL.sig and
                                        (vi < 72.44 or not sL.var)):
        spec = '\t<spectrum type="PLSuperExpCutoff2" apply_edisp="false">\n'
    else:
        spec = '\t<spectrum type="PLSuperExpCutoff2" apply_edist="%s">\n' % sL.ed
    spec += '\t<!-- Source is %s degrees away from ROI center -->\n' % dist
    ...
    if (dist > sL.roi[2]):  #if beyond ROI, shouldn't attempt to fit parameters
        ...
        if dist > sL.maxRad:
            ...
            free = True
        spec += '\t\t<parameter free="0" max="10.0" min="0.0" name="Index1" scale="-1.0" value="%s"/>\n' % i
        spec += '\t\t<parameter free="0" max="100" min="0.1" name="Expfactor" scale="1.0" value="%f"/>\n' % (
            a * 1000.)
    elif (TS < sL.sig):
        if vi < 72.44 or not sL.var:
            spec += '\t<!-- Source significance %.1f is less than specified minimum for a free source of %s -->\n' % (
            ...
            free = True
        spec += '\t\t<parameter free="0" max="10.0" min="0.0" name="Index1" scale="-1.0" value="%s"/>\n' % i
        spec += '\t\t<parameter free="0" max="100" min="0.1" name="Cutoff" scale="0.001" value="%f"/>\n' % (
            a * 1000.)
    else:
        free = True
        spec += '\t\t<parameter free="1" max="1e4" min="1e-4" name="Prefactor" scale="1e%i" value="%s"/>\n' % (
...
def LPspec(sL, f, i, p, b, dist, TS, vi):
    ...
    if dist > sL.roi[2] or (dist > sL.maxRad) or (
        ...
        spec = '\t<spectrum type="LogParabola" apply_edist="%s">\n' % sL.ed
    spec += '\t<!-- Source is %s degrees away from ROI center -->\n' % dist
    if (dist > sL.roi[2]):  #if beyond ROI, shouldn't attempt to fit parameters
        spec += '\t<!-- Source is outside ROI, all parameters should remain fixed -->\n'
...
def getPos(ft1):
    ...
    if (
            keynum == 0
    ):  #DSKEYS start numbering at 1, if this value hasn't been updated, KEYword doesn't exist
    ...
    try:
        ra, dec, rad = header[keyword].strip('CIRCLE()').split(
            ','
        )  #gets rid of the circle and parenthesis part and splits around the comma
        float(ra)
    except:
        ra, dec, rad = header[keyword].strip('circle()').split(',')
    file.close()
...
def cli():
    ...
    parser.add_argument(
        "catalog", type=str, help="Catalog file to use, can be FITS or xml.")
    parser.add_argument(
        "ev", type=str, help="Event file with ROI information in header.")
    parser.add_argument(
        "-o",
        "--outputxml",
        type=str,
    ...
    parser.add_argument(
        ...
        "--galfile",
        type=str,
        default='$(FERMI_DIR)/refdata/fermi/galdiffuse/gll_iem_v06.fits',
        help=
        "Name and location of Galactic diffuse model to use, will default to 3FGL model."
    )
    parser.add_argument(
        "-g",
        "--galname",
        ...
        default='gll_iem_v06',
        help=
        "Name of Galactic diffuse component in output model, will default to gll_iem_v06."
    )
    parser.add_argument(
        "-I",
        "--isofile",
        type=str,
        default=
        '$(FERMI_DIR)/refdata/fermi/galdiffuse/iso_P8R2_SOURCE_V6_v06.txt',
        help=
        "Name of isotropic diffuse template for output model, will default to 3FGL model."
    )
    parser.add_argument(
        "-i",
        "--isoname",
        type=str,
        default='iso_P8R2_SOURCE_V6_v06',
        help=
        "Name of isotropic diffuse component in output model, will default to iso_source_v05."
    )
    parser.add_argument(
        "-N",
        "--normsonly",
        ...
        default=False,
        help=
        "Flag to only let the normalizations of parameters be free, default is False.",
        nargs="?",
        const=True,
        choices=[
            'True', 'False', 'T', 'F', 't', 'f', 'TRUE', 'FALSE', 'true',
            'false', 1, 0
        ])
    parser.add_argument(
        "-e",
        ...
        default='',
        help=
        "Path to directory with LAT extended source templates, will default to STs default."
    ...
    parser.add_argument(
        ...
        default=-1.,
        help=
        "Radius, in degrees, from ROI center beyond which all source parameters should be fixed, will default to selection radius."
    )
    parser.add_argument(
        "-R",
        "--maxRad",
        ...
        default=None,
        help=
        "Absolute maximum radius, in degrees, from ROI center beyond which all source parameters should be fixed, even variable sources will not be freed beyond this radius, defaults to radLim value."
    )
    parser.add_argument(
        "-ER",
        "--ExtraRad",
        ...
        default=10.,
        help=
        "Radius beyond event file ROI out to which sources will be included in the model with all parameters fixed, default is 10, good for analyses starting around a few hundred MeV, can be decreased for high energy only fits."
    )
    parser.add_argument(
        "-s",
        "--sigFree",
        ...
        default=5.,
        help=
        "Average significance below which all source parameters are fixed, defaults to 5.  Note, if using the 3FGL catalog xml file as input, this is actually a cut on TS, so adjust accordingly."
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        "Flag to cast extended sources as point sources, default is False.",
        nargs="?",
        const=True,
        choices=[
            'True', 'False', 'T', 'F', 't', 'f', 'TRUE', 'FALSE', 'true',
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        "Flag to use catalog names for extended sources, default is False.",
        nargs="?",
        const=True,
        choices=[
            'True', 'False', 'T', 'F', 't', 'f', 'TRUE', 'FALSE', 'true',
    ...
    parser.add_argument(
        ...
        default=True,
        help=
        "Flag to create ds9 region file as well as the xml model, default is True.",
        choices=[
            'True', 'False', 'T', 'F', 't', 'f', 'TRUE', 'FALSE', 'true',
            'false', 1, 0
        ])
    parser.add_argument(
        ...
        default=False,
        help=
        "Flag to use a power-law modification to the Galactic diffuse model spectrum and have the index be free, default is False.",
        nargs="?",
        const=True,
        choices=[
            'True', 'False', 'T', 'F', 't', 'f', 'TRUE', 'FALSE', 'true',
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        "Flag to turn on energy dispersion for free point and extended sources, never for diffuse backgrounds, default is False.",
        nargs="?",
        const=True,
        choices=[
            'True', 'False', 'T', 'F', 't', 'f', 'TRUE', 'FALSE', 'true',
    ...
    parser.add_argument(
        ...
        default='',
        help=
        "Directory to write the output ds9 region file in if not the current working directory or if you are specifying the full path to the newly made XML file."
    )
    parser.add_argument(
        "-ON",
        "--oldNames",
        ...
        default=False,
        help=
        "Flag to use the make2FLGxml style naming convention, underscore before name and no spaces, default is False.",
        nargs="?",
        const=True,
        choices=[
            'True', 'False', 'T', 'F', 't', 'f', 'TRUE', 'FALSE', 'true',
    ...
    sL.makeModel(args.galfile, args.galname, args.isofile, args.isoname,
                 args.normsonly, args.extDir, args.radLim, args.maxRad,
                 args.ExtraRad, args.sigFree, args.varFree, args.psForce,
                 args.E2CAT, args.makeRegion, args.GIndexFree, args.edisp,
                 args.writeDir, args.oldNames)
